...
// An explicit mirror choice must never silently download from another source.
export function selectedDownload(build, manifest, source = 'github') {
  if (source === 'github') return build.url;
  if (source !== 'mirror') return null;
  const mirror = downloadSource(build, manifest);
  return mirror.mirrored ? mirror.url : null;
}
